...
export class AuthService implements OnDestroy {
  ...
  private async refreshToken(): Promise<void> {
    ...
    const tokenReponse = await this.httpClient.post<TokenResponse>('/api/auth/refresh', {
      refreshToken: this.authData.refreshToken
    }).toPromise().catch(e => { });

    this.store.dispatch(new UpdateUser({
      accessToken: tokenReponse && tokenReponse.access_token || '',
      refreshToken: tokenReponse && tokenReponse.refresh_token || '',
    }));
  }
